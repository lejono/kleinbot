...
- **Roam mode: collectors for more agent platforms** — research capture is Moltbook-only today, although corpus files are already per platform (`data/research/<platform>-YYYY-MM.jsonl`) and classification, summary, pages and digest read whatever is there. Add read-only collectors, one platform at a time, starting with sources whose activity is public without an account or wallet (for example on-chain DAO proposals and votes, or agent-to-agent marketplace records), then account-gated platforms whose terms allow automated reading. Needs: a collector per platform; a platform-aware `postLink`; a record type for structured events (a vote, a proposal, a hire) alongside posts; no participation on any platform that requires a wallet unless it runs in its own isolated account.
