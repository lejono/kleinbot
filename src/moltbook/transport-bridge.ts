...
/**
 * Handle a moltbookAction from a Claude response.
...
/**
 * Send queued cross-pollination digests to moltbook-enabled chats.
 */
export async function sendCrossPollination(transport: Transport): Promise<void> {
  ...
  for (const [jid, chatConfig] of Object.entries(chats)) {
    ...
    try {
      await transport.sendText(jid, digest);
...
function formatDigest(items: MoltbookCrossPollination[]): string {
  ...
  const sections = unique.map((item) => {
    const postUrl = `https://www.moltbook.com/post/${item.postId}`;
    const authorUrl = item.author ? `https://www.moltbook.com/u/${item.author}` : null;
    const authorLine = item.author
      ? `by ${item.author} (${authorUrl}) in r/${item.submolt}`
      : `in r/${item.submolt}`;

    return `*${item.title}*\n${authorLine}\n${item.snippet}\n${postUrl}`;
  });

  return `Moltbook intel drop:\n\n${sections.join("\n\n---\n\n")}`;
}

/**
 * Send a briefing message to all chats with "briefing": true in chats config.
 */
export async function sendBriefing(transport: Transport, message: string): Promise<void> {
  ...
  for (const [jid, chatConfig] of Object.entries(chats)) {
    if (jid === "default") continue;
    if (!(chatConfig as any).briefing) continue;

    try {
      await transport.sendText(jid, message);
      console.log(`[briefing] Sent briefing to ${jid}`);
    } catch (err: any) {
