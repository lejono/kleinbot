...
import type { ChatMessage, MessageAttachment } from "./types.js";
...
export function createSignalTransport(
  ...
  // Track known group IDs so we can distinguish DMs from groups even when
  // Signal uses base64 service IDs instead of phone numbers for DMs.
  ...
  return {
    ...
    async start(onMessage: MessageHandler) {
      await rpc.connect((method, params) => {
        ...
        // Accept messages with text OR attachments (image-only messages are valid)
        if (!dataMessage || (!dataMessage.message && !dataMessage.attachments?.length)) return;
        ...
        // Deduplicate — signal-cli can deliver the same message via both notification formats
        const msgId = `${envelope.sourceNumber}-${envelope.timestamp}`;
        ...
        if (dataMessage.attachments?.length) {
          ...
          chatMessage.attachments = dataMessage.attachments.map((att: any) => ({
            ...
            size: att.size || 0,
            localPath: att.id
              ? path.join(attachmentsDir, String(att.id))
              : att.remoteId
                ? path.join(attachmentsDir, String(att.remoteId))
                : "",
      ...
      // Pre-populate known groups (must happen after connect)
      try {
        const groups = (await rpc.call("listGroups", { account })) as any[];
        if (groups) {
          for (const g of groups) {
            const gid = g.id || g.groupId;
            if (gid) knownGroupIds.add(gid);
          }
          console.log(`[signal] Loaded ${knownGroupIds.size} known groups`);
        }
      } catch (err: any) {
        console.warn("[signal] Failed to pre-load groups:", err.message);
      }
    },
    ...
    async sendText(chatId, text) {
      ...
      const isGroup = knownGroupIds.has(chatId);
    ...
    isDm(chatId) {
      return !knownGroupIds.has(chatId);
    },
    ...
    async fetchGroupDescription(chatId) {
      if (!knownGroupIds.has(chatId)) return null;
      try {
        const groups = (await rpc.call("listGroups", { account })) as any[];
